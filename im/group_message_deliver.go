...
type GroupMessageDeliver struct {
	...
	dt chan *core.AppMessage //dispatch 群组消息

	cb_mutex            sync.Mutex                    //callback变量的锁
	id                  int64                         //自增的callback id
	callbacks           map[int64]chan *core.Metadata //返回保存到ims的消息id
...
func NewGroupMessageDeliver(root string) *GroupMessageDeliver {
	...
	storage.dt = make(chan *core.AppMessage, 1000)
	storage.callbacks = make(map[int64]chan *core.Metadata)
...
func (storage *GroupMessageDeliver) ReadMessage(file *os.File) *core.Message {
	//校验消息起始位置的magic
	var magic int32
	...
	msg := core.ReceiveStorageMessage(file)
	if msg == nil {
		return msg
	}
...
func (storage *GroupMessageDeliver) LoadMessage(msg_id int64) *core.Message {
	storage.mutex.Lock()
...
//save without lock
func (storage *GroupMessageDeliver) saveMessage(msg *core.Message) int64 {
	...
	body := msg.ToData()
	var msg_len int32 = core.MSG_HEADER_SIZE + int32(len(body))
	binary.Write(buffer, binary.BigEndian, msg_len)

	core.WriteHeader(int32(len(body)), int32(msg.Seq), byte(msg.Cmd),
	...
	log.Info("save message:", core.Command(msg.Cmd), " ", msgid)
	return msgid
...
func (storage *GroupMessageDeliver) DoCallback(msgid int64, meta *core.Metadata) {
	storage.cb_mutex.Lock()
	defer storage.cb_mutex.Unlock()
...
func (storage *GroupMessageDeliver) AddCallback(msgid int64, ch chan *core.Metadata) int64 {
	storage.cb_mutex.Lock()
	defer storage.cb_mutex.Unlock()
	storage.id += 1
...
func (storage *GroupMessageDeliver) SaveMessage(msg *core.Message, ch chan *core.Metadata) int64 {
	storage.mutex.Lock()
	defer storage.mutex.Unlock()
...
//device_ID 发送者的设备ID
func (storage *GroupMessageDeliver) sendMessage(appid int64, uid int64, sender int64, device_ID int64, msg *core.Message) bool {

	PublishMessage(appid, uid, msg)

	route := app_route.FindRoute(appid)
	if route == nil {
		log.Warningf("can't send message, appid:%d uid:%d Cmd:%s", appid, uid, core.Command(msg.Cmd))
		return false
	}
...
func (storage *GroupMessageDeliver) sendGroupMessage(gm *core.PendingGroupMessage) (*core.Metadata, bool) {
	msg := &core.IMMessage{Sender: gm.Sender, Receiver: gm.Gid, Timestamp: gm.Timestamp, Content: gm.Content}
	...
	for _, mb := range batch_members {
		...
		for i := 0; i < len(r); i += 2 {
			...
			mm := &core.Message{Cmd: core.MSG_GROUP_IM, Version: core.DEFAULT_VERSION,
				Flag: core.MESSAGE_FLAG_PUSH, Body: msg, Meta: meta}
			...
			if member == gm.Sender {
				...
				metadata.PrevSyncKey = prev_msgid
			}
		}
	}
...
func (storage *GroupMessageDeliver) sendPendingMessage() {
	...
	for {
		...
		if msg.Cmd != core.MSG_PENDING_GROUP_MESSAGE {
			continue
		}

		gm := msg.Body.(*core.PendingGroupMessage)
...
func (storage *GroupMessageDeliver) DispatchMessage(msg *core.AppMessage) {
