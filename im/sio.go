...
func ReadBinaryMesage(b []byte) *core.Message {
	reader := bytes.NewReader(b)
	return core.ReceiveClientMessage(reader)
}
...
func ReadWebsocketMessage(conn *websocket.Conn) *core.Message {
...
func SendWebsocketBinaryMessage(conn *websocket.Conn, msg *core.Message) error {
	w, err := conn.NextWriter(websocket.BinaryMessage)
	if err != nil {
		log.Info("get next writer fail")
		return err
	}
	err = core.SendMessage(w, msg)
	if err != nil {
		return err
	}
	err = w.Close()
