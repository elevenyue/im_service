...
func (route *Route) RemoveClient(c *Client) bool {
	route.mutex.Lock()
	defer route.mutex.Unlock()
	if set, ok := route.clients[c.uid]; ok {
		set.Remove(c)
		if set.Count() == 0 {
			delete(route.clients, c.uid)
		}
		return true
	}
...
func (route *Route) GetUserIDs() core.IntSet {
	return core.NewIntSet()
}
