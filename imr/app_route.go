...
func (app_route *AppRoute) FindOrAddRoute(appid int64, MakeRoute func(appid int64) *Route) *Route {
	app_route.mutex.Lock()
	defer app_route.mutex.Unlock()
	if route, ok := app_route.apps[appid]; ok {
		return route
	}
	route := MakeRoute(appid)
...
func (app_route *AppRoute) GetUsers() map[int64]core.IntSet {
	app_route.mutex.Lock()
	defer app_route.mutex.Unlock()

	r := make(map[int64]core.IntSet)
