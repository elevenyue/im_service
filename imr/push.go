...
//离线消息入apns队列
func (client *Client) PublishPeerMessage(appid int64, im *core.IMMessage) {
	conn := redis_pool.Get()
	defer conn.Close()
...
func (client *Client) PublishGroupMessage(appid int64, receivers []int64, im *core.IMMessage) {
	conn := redis_pool.Get()
	defer conn.Close()
...
func (client *Client) PublishCustomerMessage(appid, receiver int64, cs *core.CustomerMessage, cmd int) {
	conn := redis_pool.Get()
	defer conn.Close()
