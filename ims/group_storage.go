...
func (storage *GroupStorage) SaveGroupMessage(appid int64, gid int64, device_id int64, msg *core.Message) (int64, int64) {
	storage.mutex.Lock()
	...
	last_id := index.LastId
	last_batch_id := index.LastBatchId
	last_seq_id := index.LastSeqId
	...
	m := &core.Message{Cmd: core.MSG_GROUP_OFFLINE, Body: off}
...
func (storage *GroupStorage) setGroupIndex(appid int64, gid int64, gi *GroupIndex) {
	...
	if gi.LastId > storage.last_id {
		storage.last_id = gi.LastId
	}
}
...
//获取所有消息id大于msgid的消息
//ts:入群时间
func (storage *GroupStorage) LoadGroupHistoryMessages(appid int64, uid int64, gid int64, msgid int64, ts int32, limit int) ([]*core.EMessage, int64) {
	log.Infof("load group history message:%d %d", msgid, ts)
	msg_index := storage.GetGroupIndex(appid, gid)
	last_id := msg_index.LastId

	var last_msgid int64
	c := make([]*core.EMessage, 0, 10)
	...
	for last_id > 0 {
		...
		if msgid == 0 && m.Cmd == core.MSG_GROUP_IM {
			//不取入群之前的消息
			im := m.Body.(*core.IMMessage)
			if im.Timestamp < ts {
				break
			}
		}
		c = append(c, &core.EMessage{MsgId: off.Msgid, DeviceId: off.DeviceId, Msg: m})
...
func (storage *GroupStorage) execMessage(msg *core.Message, msgid int64) {
	if msg.Cmd == core.MSG_GROUP_IM_LIST {
		off := msg.Body.(*core.GroupOfflineMessage)
		gi := &GroupIndex{off.Msgid, msgid, 0, 0}
		storage.setGroupIndex(off.Appid, off.Receiver, gi)
	} else if msg.Cmd == core.MSG_GROUP_OFFLINE {
		off := msg.Body.(core.IOfflineMessage).Body()
		index := storage.getGroupIndex(off.Appid, off.Receiver)
		last_id := msgid
		last_batch_id := index.LastBatchId
		last_seq_id := index.LastSeqId + 1
		if last_seq_id%BATCH_SIZE == 0 {
			last_batch_id = msgid
		}
