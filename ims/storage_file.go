...
func (storage *StorageFile) ReadMessage(file *os.File) *core.Message {
	//校验消息起始位置的magic
	var magic int32
...
func (storage *StorageFile) LoadMessage(msg_id int64) *core.Message {
	storage.mutex.Lock()
...
func (storage *StorageFile) WriteMessage(file io.Writer, msg *core.Message) {
	buffer := new(bytes.Buffer)
	binary.Write(buffer, binary.BigEndian, int32(MAGIC))
	core.WriteMessage(buffer, msg)
	binary.Write(buffer, binary.BigEndian, int32(MAGIC))
	buf := buffer.Bytes()
...
//save without lock
func (storage *StorageFile) saveMessage(msg *core.Message) int64 {
	...
	master.ewt <- &core.EMessage{MsgId: msgid, Msg: msg}
