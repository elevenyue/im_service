...
func (storage *PeerStorage) SavePeerMessage(appid int64, uid int64, device_id int64, msg *core.Message) (int64, int64) {
	storage.mutex.Lock()
	...
	off := &core.OfflineMessage4{}
	off.Appid = appid
	off.Receiver = uid
	off.Msgid = msgid
	off.DeviceId = device_id
	off.SeqId = last_seq_id + 1
	off.PrevMsgid = last_id
	off.PrevPeerMsgid = last_peer_id
	off.PrevBatchMsgid = last_batch_id

	var flag int
	if storage.isGroupMessage(msg) {
		flag = core.MESSAGE_FLAG_GROUP
	}
	m := &core.Message{Cmd: core.MSG_OFFLINE_V4, Flag: flag, Body: off}
...
func (storage *PeerStorage) SavePeerGroupMessage(appid int64, members []int64, device_id int64, msg *core.Message) []int64 {
...
//总消息数限制在limit
func (storage *PeerStorage) LoadHistoryMessages(appid int64, receiver int64, sync_msgid int64, group_limit int, limit int) ([]*core.EMessage, int64, bool) {
	var last_msgid int64
	last_id, _ := storage.GetLastMessageID(appid, receiver)
	messages := make([]*core.EMessage, 0, 10)
	for {
		if last_id == 0 {
			break
		}
		...
		var off *core.OfflineMessage
		if ioff, ok := msg.Body.(core.IOfflineMessage); ok {
			off = ioff.Body()
		} else {
		...
		if msg.Cmd != core.MSG_GROUP_IM &&
			msg.Cmd != core.MSG_GROUP_NOTIFICATION &&
			msg.Cmd != core.MSG_IM &&
			msg.Cmd != core.MSG_CUSTOMER &&
			msg.Cmd != core.MSG_CUSTOMER_SUPPORT &&
			msg.Cmd != core.MSG_SYSTEM {
			if group_limit > 0 && len(messages) >= group_limit {
				last_id = off.PrevPeerMsgid
			} else {
		...
		emsg := &core.EMessage{MsgId: off.Msgid, DeviceId: off.DeviceId, Msg: msg}
...
func (storage *PeerStorage) LoadHistoryMessagesV3(appid int64, receiver int64, sync_msgid int64, limit int, hard_limit int) ([]*core.EMessage, int64, bool) {
	var last_msgid int64
	...
	for {
		...
		last_batch_id = off.PrevBatchMsgid
...
func (storage *PeerStorage) LoadLatestMessages(appid int64, receiver int64, limit int) []*core.EMessage {
	last_id, _ := storage.GetLastMessageID(appid, receiver)
	messages := make([]*core.EMessage, 0, 10)
	for {
		if last_id == 0 {
...
func (client *PeerStorage) isGroupMessage(msg *core.Message) bool {
	return msg.Cmd == core.MSG_GROUP_IM || msg.Flag&core.MESSAGE_FLAG_GROUP != 0
}

func (client *PeerStorage) isSender(msg *core.Message, appid int64, uid int64) bool {
...
func (storage *PeerStorage) GetNewCount(appid int64, uid int64, last_received_id int64) int {
	...
	var off *core.OfflineMessage
	if ioff, ok := off_m.Body.(core.IOfflineMessage); ok {
		off = ioff.Body()
	} else {
	...
	if last_seq_id < off.SeqId {
		return 0
	}

	return int(last_seq_id - off.SeqId)
}
...
func (storage *PeerStorage) execMessage(msg *core.Message, msgid int64) {
	if msg.Cmd == core.MSG_OFFLINE {
		off := msg.Body.(core.IOfflineMessage).Body()
		ui := &UserIndex{off.Msgid, msgid, msgid, 0, 0}
		storage.setPeerIndex(off.Appid, off.Receiver, ui)
	} else if msg.Cmd == core.MSG_OFFLINE_V2 {
		off := msg.Body.(core.IOfflineMessage).Body()
		last_peer_id := msgid
		if (msg.Flag & core.MESSAGE_FLAG_GROUP) != 0 {
			_, last_peer_id = storage.getLastMessageID(off.Appid, off.Receiver)
		}
		ui := &UserIndex{off.Msgid, msgid, last_peer_id, 0, 0}
		storage.setPeerIndex(off.Appid, off.Receiver, ui)
	} else if msg.Cmd == core.MSG_OFFLINE_V3 || msg.Cmd == core.MSG_OFFLINE_V4 {
		off := msg.Body.(core.IOfflineMessage).Body()
		last_peer_id := msgid
