...
func (storage *Storage) execMessage(msg *core.Message, msgid int64) {
...
func (storage *Storage) SaveSyncMessageBatch(mb *core.MessageBatch) error {
	id := mb.FirstId
	//all message come from one block
	for _, m := range mb.Msgs {
		emsg := &core.EMessage{id, 0, m}
...
func (storage *Storage) SaveSyncMessage(emsg *core.EMessage) error {
	storage.mutex.Lock()
	defer storage.mutex.Unlock()
...
func (storage *Storage) LoadSyncMessagesInBackground(cursor int64) chan *core.MessageBatch {
	c := make(chan *core.MessageBatch, 10)
	go func() {
		defer close(c)
		...
		for {
			...
			batch := &core.MessageBatch{Msgs: make([]*core.Message, 0, BATCH_COUNT)}
			for {
