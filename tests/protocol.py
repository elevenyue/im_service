...
class AuthenticationToken:
    def __init__(self):
        self.token = ""
        self.PlatformId = PLATFORM_ANDROID
...
def send_message(cmd, seq, msg, sock):
    if cmd == MSG_AUTH_TOKEN:
        b = struct.pack("!BB", msg.PlatformId, len(msg.token)) + bytes(msg.token, "utf-8") + struct.pack("!B", len(msg.device_id)) + bytes(msg.device_id, "utf-8")
        length = len(b)
